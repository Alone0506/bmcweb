...
// sensorPath and sensorName generated by the IPMI host in sensor-gen.cpp
inline std::unordered_map<std::string, std::string> ipmiSensorPathName;

template <typename Callback>
inline void loadIpmiSensorPathName(
    ...
    dbus::utility::async_method_call(
        [asyncResp, callback = std::forward<Callback>(callback)](
            ...
            const std::unordered_map<std::string, std::string>&
                sensorPathName) mutable {
            if (ec)
            {
                BMCWEB_LOG_ERROR("Call ipmi host method error: {}", ec);
                messages::internalError(asyncResp->res);
                return;
            }

            ipmiSensorPathName = sensorPathName;
            callback();
        },
        "xyz.openbmc_project.Ipmi.Host", "/xyz/openbmc_project/Ipmi",
        "xyz.openbmc_project.Ipmi.SensorList", "getSensorList");
}
                ...
                Callback&& callback)
{
    ...
    {
        auto processIpmiSensors = [callback = std::forward<Callback>(
                                       callback)]() mutable {
            const std::shared_ptr<std::set<std::string>> culledSensorList =
                std::make_shared<std::set<std::string>>();
            for (auto& [path, name] : ipmiSensorPathName)
            {
                if (name.empty())
                {
                ...
                culledSensorList->insert(path);
            }
            BMCWEB_LOG_DEBUG("Async IPMI {} sensors", culledSensorList->size());
            callback(culledSensorList);
        };

        if (ipmiSensorPathName.size() == 0)
        {
            loadIpmiSensorPathName(asyncResp, std::move(processIpmiSensors));
        }
        else
        {
            processIpmiSensors();
        ...
        *crow::connections::systemBus, connectionName, sensorPath, "",
        [asyncResp,
         sensorPath](const boost::system::error_code& ec,
            ...
            if constexpr (BMCWEB_IPMI_STATIC_SENSOR)
            {
                name = ipmiSensorPathName[sensorPath];
            }
         ...
         sensorPath](const boost::system::error_code& ec,
            ...
            if (ec == boost::system::errc::io_error)
            {
                if constexpr (BMCWEB_IPMI_STATIC_SENSOR)
                {
                    ...
                    if (ipmiSensorPathName.count(sensorPath) == 0)
                    {
                        asyncResp->res.jsonValue["Name"] = nameType.first;
                    }
                    else
                    {
            ...
            if constexpr (BMCWEB_IPMI_STATIC_SENSOR)
            {
                if (ipmiSensorPathName.empty())
                {
                    loadIpmiSensorPathName(asyncResp, [asyncResp, sensorPath,
                                                       subtree]() mutable {
